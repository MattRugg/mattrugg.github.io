...
//
// Modified from: https://www.shadertoy.com/view/4dfSDj
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec4 pos = vec4(0.);
	...
	for (int i=0; i<50; ++i)
	{
    pos = vec4(p.x, 0.35*fbm4(0.5*vec2(eye.x+p.x, z+off)), eye.z+z, 1.);
    h = (vpmat*pos).y - p.y;
	...
	col = 0.1*sqrt(clamp(acc, 0., 1.));
  
  // Background
  vec3 bgTop = vec3(0.75, 0.75, 0.75); // darker top color
  vec3 bgBottom = vec3(241.0/255.0, 241.0/255.0, 241.0/255.0); // lighter bottom color

  // Create a vertical gradient from bgBottom (bottom) to bgTop (top)
  vec3 bkg = mix(bgBottom, bgTop, uv.y);
  col += bkg;

	col *= (0.15 + 0.85*(1.0-exp(-(1.0)*30.0)));
